...
// Ported verbatim from VRM-Qavatars' avatarGenerator.cpp -- this is dense,
// tested bone-name-mapping logic (VRM's enum-tagged bone list -> Unity's
// Mecanim human bone names) where inventing a substitute would be actively
// wrong, not just incomplete. No changes beyond include paths and the
// logger.
...
namespace AssetLib::Generators
{
    ...
    UnityEngine::Avatar* AvatarGenerator::Generate(const VRMC_VRM_0_0::Vrm& vrm, std::vector<AssetLib::Structure::Node*> bones, UnityEngine::GameObject* root)
    {
        ...
        try
        {
            using BuildHumanAvatarFn = UnityEngine::Avatar* (*)(UnityEngine::GameObject*, UnityEngine::HumanDescription*);
            static auto buildHumanoid = reinterpret_cast<BuildHumanAvatarFn>(il2cpp_functions::resolve_icall("UnityEngine.AvatarBuilder::BuildHumanAvatarInternal_Injected"));
